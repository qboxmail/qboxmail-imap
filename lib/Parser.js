...
var CH_LF = 10,
  ...
  RE_LITERAL = /\{(\d+)\}$/,
  RE_UNTAGGED =
    /^\* (?:(OK|NO|BAD|BYE|FLAGS|ID|LIST|XLIST|ACL|MYRIGHTS|LSUB|SEARCH|CAPABILITY|NAMESPACE|PREAUTH|SORT|THREAD|ESEARCH|QUOTA|QUOTAROOT)|(\d+) (EXPUNGE|FETCH|RECENT|EXISTS))(?:(?: \[([^\]]+)\])?(?: (.+))?)?$/i,
  RE_METADATA = /^\* METADATA (.*)/gim,
...
Parser.prototype._parse = function (data) {
  ...
  while (i < datalen) {
    ...
    if (idxlf === -1) {
      ...
      if (this._buffer.indexOf("* METADATA ") === 0) {
...
Parser.prototype._resUntagged = function () {
  ...
  } else if ((m = RE_METADATA.exec(this._buffer))) {
    this._buffer = "";
    this.emit("untagged", {
      type: "metadata",
      num: undefined,
  ...
  for (; i < len; ++i) {
    if (hasMetadata) {
      if (
        buffer[i] === ch &&
        (!buffer[i + 1] ||
          buffer[i + 1] === "A".charCodeAt(0) ||
          buffer[i + 1] === "*".charCodeAt(0) ||
          buffer[i + 1] === "+".charCodeAt(0))
      ) {
        r = i;
        break;
...
function parseBoxList(text, literals) {
  ...
  text = text.replace(
    RE_LIST_REPONSE,
    function (match, p1, boxName, offset, str) {
      if (!boxName) {
        return str;
      }
...
function parseStatus(text, literals) {
  ...
  text = text.replace(
    RE_STATUS_RESPONSE,
    function (match, boxName, p2, offset, str) {
      if (!boxName) {
        return str;
      }
      if (boxName[0] === '"' && boxName[boxName.length - 1] === '"') {
...
function shouldQuoteXMailboxValue(value) {
  if (!value) return false; // no value to normalize
  if (value === LITPLACEHOLDER) return false; // literal already handled elsewhere
  if (/^nil$/i.test(value)) return false; // IMAP NIL should stay bare
  var first = value[0];
  if (first === '"' || first === "(" || first === "{") return false; // already a quoted string, list, or literal
  return value.indexOf("[") !== -1 || value.indexOf("]") !== -1;
}

function quoteAsString(value) {
  // Escape backslashes and quotes so the value remains a single quoted atom.
  return '"' + value.replace(/\\/g, "\\\\").replace(/"/g, '\\"') + '"';
}

function normalizeFetchXMailbox(text) {
  // Quote unquoted X-MAILBOX values with brackets so parseExpr treats them as atoms.
  return text.replace(
    /(\bX-MAILBOX\s+)([^)\s]+)/gi,
    function (match, prefix, value) {
      if (!shouldQuoteXMailboxValue(value)) return match;
      return prefix + quoteAsString(value);
    }
  );
}

function parseFetch(text, literals, seqno) {
  text = normalizeFetchXMailbox(text);
...
function decodeBytes(buf, encoding, offset, mlen, pendoffset, state, nextBuf) {
  ...
  if (jsencoding.encodingExists(encoding)) {
    ...
    if (!isPartial && nextBuf) {
      ...
      var lookahead,
        lookaheadBuf = new Buffer(buf.length + nextBuf.length);
...
exports.normalizeFetchXMailbox = normalizeFetchXMailbox;
