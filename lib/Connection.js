...
Connection.prototype.getAcl = function (folderName, cb) {
  this._enqueue('GETACL "' + escape(utf7.encode("" + folderName)) + '"', cb);
};

Connection.prototype.myRights = function (folderName, cb) {
  this._enqueue('MYRIGHTS "' + escape(utf7.encode("" + folderName)) + '"', cb);
};

Connection.prototype.listRights = function (folderName, identifier, cb) {
  this._enqueue(
    'LISTRIGHTS "' +
      escape(utf7.encode("" + folderName)) +
      '" "' +
      escape(utf7.encode("" + identifier)) +
      '"',
    cb
  );
};

Connection.prototype.setAcl = function (folderName, username, permissions, cb) {
  this._enqueue(
    'SETACL "' +
      escape(utf7.encode("" + folderName)) +
      '" ' +
...
Connection.prototype.deleteAcl = function (folderName, username, cb) {
  this._enqueue(
    'DELETEACL "' + escape(utf7.encode("" + folderName)) + '" ' + username,
    cb
  );
};

Connection.prototype.getMetadata = function (folderName, attribute, cb) {
  this._enqueue(
    'GETMETADATA "' +
      escape(utf7.encode("" + folderName)) +
      '" "' +
      escape(utf7.encode("" + attribute)) +
      '"',
    cb
  );
};

Connection.prototype.setMetadata = function (folderName, attribute, cb) {
  this._enqueue(
    'SETMETADATA "' +
      escape(utf7.encode("" + folderName)) +
      '" (' +
...
Connection.prototype._fetch = function (which, uids, options) {
  ...
  if (options) {
    ...
    if (
      options.preview &&
      (this.serverSupports("PREVIEW=FUZZY") || this.serverSupports("PREVIEW"))
    )
      fetching.push("PREVIEW");
...
Connection.prototype._resUntagged = function (info) {
  ...
  if (type === "bye") this._sock.end();
    ...
    let acl_res = [];
    if (info.text.trim()[0] === '"') {
      acl_res = info.text.replace(/^".*" /, "").split(" ");
    } else {
      acl_res = info.text.split(" ");
      acl_res.shift();
    }
    this._curReq.cbargs.push(acl_res);
  } else if (type === "myrights") {
    let acl_res = info.text.trim().split(" ");
    this._curReq.cbargs.push(acl_res[acl_res.length - 1].trim());
  } else if (type === "metadata") {
    let line = info.text.trim();
    let m = line.match(/\((.*?)\)/);
    let key_value = m[1].replace(/\{.*?\}/g, "");
    ...
    this._curReq.cbargs.push(metadata_res);
  } else if (type === "fetch") {
...
Connection.prototype._login = function () {
  ...
  self._enqueue("CAPABILITY", function () {
    ...
    if (self.state === "connected") {
      ...
      if (self.serverSupports("ID") && typeof self._config.id === "object") {
        self.id(self._config.id, function (err, res) {
          delete self._config.id;
          return reentry(err);
        });
      }
...
Connection.prototype._processQueue = function () {
  ...
  if (this._curReq && this._curReq.literalAppendData) {
